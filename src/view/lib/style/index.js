...
const defaultElemenst = props => {
  ...
  if (props._mediaScreen) {
    if (Array.isArray(props._mediaScreen)) {
      if (props._mediaScreen.length > 0) {
        for (const element of props._mediaScreen) {
      ...
      render += `@media only screen and (${
        props._mediaScreen.dimensions
      }){${defaultElemenst(props._mediaScreen.style)}${
        props._mediaScreen.pureStyle ? props._mediaScreen.pureStyle : ""
      }}`;
  }
