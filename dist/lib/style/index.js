...
const generateStyle = props => {
  ...
  let keyProps = Object.keys(props).join("$"),
    ...
    if (shorts[val]) {
      ...
      if (regVal.test(shorts[val])) return shorts[val].replace(regVal, props[val]) + ";";
      if (typeof props[val] === "boolean" && props[val]) return shorts[val] + ";";
    ...
    if (shortFuncs[val]) {
      if (typeof props[val] === "object") {
        if (val === "that") return `${shortFuncs[val]}${props[val + "Query"] || ""}{${generateStyle(props[val])}}`;
        ...
        return `${shortFuncs[val]}${props[val + "Query"] || ""} ${props[val + "EffectEl"] || ""}{${generateStyle(props[val])}}`;
      }
    }
